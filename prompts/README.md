...
`l2_errors.txt`. Your notebook loads its own by default (`PROMPT_FILE = "../prompts/" +
TRACK + ".txt"`).
...
2. **Save each version as its own file** — `cefr.txt`, `cefr_v2_added_definitions.txt`,
   `cefr_v3_stricter.txt` — and point `PROMPT_FILE` in the notebook at the one you want.
   This keeps a record of what you tried, which is exactly what the "Prompt iterations"
   table in your report needs.
